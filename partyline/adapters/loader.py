...
def _manifest(path: Path) -> dict:
    ...
    if "write_paths" in manifest:
        raise ValueError("adapter write_paths is obsolete; writable scope uses recorded grants")
    ...
    if not isinstance(value, list) or not all(isinstance(arg, str) for arg in value):
        raise ValueError("adapter fence_args must be an array of strings")
