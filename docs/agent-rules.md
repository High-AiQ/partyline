...
| Refuse to run tests unless `memory.max` inside the transient scope or inherited child `RLIMIT_AS` proves the cap; put every Linux attached process in its own verified systemd scope (default 8G, configurable with `PARTYLINE_PROCESS_MEMORY_LIMIT`) | Run a suite or attached CLI when its effective memory cap cannot be verified |
