...
| Check at boot and in `partyline doctor` that the Partyline unit has `OOMPolicy=continue` and a finite `MemoryMax` below host RAM; verify the attached-process scope by reading `memory.max` | Rely on an operator note or an unverified systemd setting to protect the service |
