...
| Ensure an adversarial review in a throwaway worktree pinned to the exact SHA is completed before accepting — the same bar for a child line's branch/report and a same-line worker's hand-off (`skills/adversarial-review/SKILL.md`) | Review on a shared workbench or a mutable branch, or accept a report or a delegate's "done" without the review having happened |
